function topng --description "Convert image file(s) to PNG into a specific output folder"
    # ---- defaults ----
    set -l strip 1
    set -l keep_name 0
    ...
    if test (count $args) -lt 2
        echo "Usage: topng [--no-strip] [--keep-name] <source-file-or-dir>... <output-dir>"
        return 1
    end
    ...
    for n in (seq (count $files))
        ...
        if test $keep_name -eq 1
            set out_name "$filename.png"
        else
            set out_name (string replace -r '\.[^.]+$' '.png' "$filename")
        end
    ...
    echo "Converting $to_convert files to PNG..."
